...
fn solve(segments: Vec<(Segment, Segment)>) {
    for (s1, s2) in segments {
        let base = s2.p2 - s2.p1;
        let d1 = base.cross(s1.p1 - s2.p1).abs();
        let d2 = base.cross(s1.p2 - s2.p1).abs();
        let t = d1 / (d1 + d2);
        let x = s1.p1 + (s1.p2 - s1.p1) * t;
        println!("{:.10} {:.10}", x.x, x.y);
    }
}
    ...
    let segments = (0..n)
        .map(|_| {
            ...
            (
                ...
                Segment::new(
                    Point::new(points[4], points[5]),
                    Point::new(points[6], points[7]),
                ),
            )
        })
        .collect();
